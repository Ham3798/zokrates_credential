...
mod tests {
    ...
    fn create_credential() {
        ...
        if let Value::Object(obj) = credential_serialized {
            for (key, value) in obj.iter() {
                ...
                let hashed_decimal_str = hashed.iter()
        ...
        // 파일에 JSON 데이터 쓰기
        let mut file = File::create("./zok/issuer/memo.json").expect("Failed to create file");
    ...
    fn load_credential_hash() -> Vec<String> {
        // 파일로부터 JSON 데이터 읽기
        let data = fs::read_to_string("./zok/issuer/memo.json").expect("Unable to read file");
    ...
    fn create_signature_and_pk() {
        ...
        let witness_file_path = "witness_values.json";
        ...
        // Python 스크립트 실행
        // signature 및 public key 생성 스크립트.
        ...
        let output = Command::new("python3")
            ...
            .arg(witness_file_path) // 파이썬 스크립트에 witness 파일 경로를 인자로 전달
            .output()
