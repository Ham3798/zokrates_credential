...
fn write_signature_for_zokrates_cli(
    pk: &PublicKey,
    sig: &(Point, BigInt),
    msg: &[u8],
    path: &str,
    ...
    let args = format!(
        "{} {} {} {} {}",
        sig_r.x, sig_r.y, sig_s, pk.p.x.n, pk.p.y.n,
    );
    ...
    let m0 = &msg_hex[..64];
    let m1 = &msg_hex[64..];
    let b0: Vec<String> = m0
        .as_bytes()
        .chunks(8)
        .map(|chunk| {
            u64::from_str_radix(std::str::from_utf8(chunk).unwrap(), 16)
                .unwrap()
                .to_string()
        })
        .collect();
    let b1: Vec<String> = m1
        .as_bytes()
        .chunks(8)
        .map(|chunk| {
            u64::from_str_radix(std::str::from_utf8(chunk).unwrap(), 16)
